...
<!-- Navigation-->
      ...
      <c:set var = "pname1" value = "${pname1}" />
        ...
        <section class="py-5">
        <c:if test = "${ID != null}"> 
        <h1 align="center"><%=(String) session.getAttribute("nickname")%>님의 장바구니 목록</h1>
        </c:if>
       </br>
       ...
       <div class="container px-4 px-lg-5 mt-5">
               ...
               %> 
                  ...
                  <c:forEach var="cnt" begin="1" end="${quantity}" step="1">  
                       <% quantity++; %> 
                  
                    <div class="col mb-5">
                        <div class="card h-100">
                            <!-- Product image-->
                            <a href="item.do?pname=<%=(String) session.getAttribute("name"+ String.valueOf(quantity))%>">
                            <img class="card-img-top" src=<%=(String) session.getAttribute("img"+ String.valueOf(quantity))%> width="350", height="400" />                            
                            <!-- Product details-->
                            <div class="card-body p-4">
                                <div class="text-center">
                                    <!-- Product name-->
                                    <h5 class="fw-bolder"><%= quantity %>. <%=(String) session.getAttribute("name"+ String.valueOf(quantity))%></h5>
                                     <!-- Product price-->
                                    
                                    <%=(Integer) session.getAttribute("price"+ String.valueOf(quantity)) %> 원
                                    
                                </div>
                                 <div class="d-flex justify-content-center small text-warning mb-2">
                                        <div class>남은 수량 <%=(Integer) session.getAttribute("count"+ String.valueOf(quantity)) %> 개</div>
                            ...
                            <div class="card-footer p-4 pt-0 border-top-0 bg-transparent">
	                                <div class="text-center">
	                       			 <a class="btn btn-outline-dark mt-auto" href="item.do?pname=<%=(String) session.getAttribute("name"+ String.valueOf(quantity))%>">구매하기</a>
	                                 <a class="btn btn-outline-dark mt-auto" href="cartdelete.do?ppname=<%=(String) session.getAttribute("name"+ String.valueOf(quantity))%>">삭제</a>
	                                </div>
                            </div>
                        </div>
                    </div>
